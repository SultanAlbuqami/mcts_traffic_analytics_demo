...
def build_diagnostic_report(accidents: pd.DataFrame, model_df: pd.DataFrame) -> str:
    ...
    region_diag["severe_rate"] = (region_diag["severe"] / region_diag["accidents"]).replace([np.inf, np.nan], 0.0)
    ...
    road_type_diag["severe_rate"] = (road_type_diag["severe"] / road_type_diag["accidents"]).replace([np.inf, np.nan], 0.0)
    ...
    lighting_diag["severe_rate"] = (lighting_diag["severe"] / lighting_diag["accidents"]).replace([np.inf, np.nan], 0.0)
    ...
    if "weather" in acc.columns:
        ...
        weather_diag["severe_rate"] = (weather_diag["severe"] / weather_diag["accidents"]).replace([np.inf, np.nan], 0.0)

    road_day_diag = pd.DataFrame()
    diag_cols = [c for c in ["region", "road_type", "daily_volume", "p95_speed", "total_violations", "violations_per_1000_volume", "low_visibility_hours", "rain_hours", "fog_hours", "severe_rate", "has_fatality"] if c in model_df.columns]
    if diag_cols:
        road_day_diag = model_df[diag_cols].copy()

    signals: list[tuple[str, float]] = []
    if "has_fatality" in road_day_diag.columns:
        for column in ["daily_volume", "p95_speed", "total_violations", "violations_per_1000_volume", "low_visibility_hours", "rain_hours", "fog_hours", "severe_rate"]:
    ...
    if {"road_id", "date", "region", "city", "road_type", "accidents", "fatalities", "severe_rate"}.issubset(observed_risk.columns):
    ...
    lines.append("")
    lines.append("This report supports diagnostic and root-cause style analysis for the solution outputs.")
    lines.append("")
    ...
    if top_signals:
        for feature, corr in top_signals:
            ...
            lines.append(f"- {feature}: corr={corr:.3f} ({direction} association with fatality flag)")
    else:
    ...
    if observed_hotspots.empty:
        ...
        lines.append("| Road | Date | Region | City | Type | Accidents | Fatalities | Severe Rate |")
        lines.append("|---|---|---|---|---|---:|---:|---:|")
        for _, row in observed_hotspots.iterrows():
            road_day = pd.to_datetime(row["date"]).date().isoformat() if pd.notna(row["date"]) else ""
            lines.append(
    ...
    lines.append("- Roads with elevated severe rates should be reviewed alongside exposure, speed profile, and violation intensity rather than accident count alone.")
    lines.append("- Low-visibility and high-speed contexts are useful diagnostic variables for targeted interventions and further field investigation.")
    lines.append("- Regional differences in severe rate suggest operational, infrastructure, enforcement, or contextual effects that need deeper investigation with real data.")
    return "\n".join(lines)
